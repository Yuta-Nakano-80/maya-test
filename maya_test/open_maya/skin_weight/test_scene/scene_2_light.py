...
def create_cylinder(root_name, z=0, x=0):
    ...
    for i in range(count):
        ...
        cylinder = cmds.polyCylinder(n=name, radius=1.0, sy=5, h=5,
                                     subdivisionsAxis=2, subdivisionsX=3, subdivisionsHeight=1)[0]
    ...
    for i in range(6):
        joint = cmds.joint(name="joint_{}".format(i), p=(x, 0, -4 * i))
        if i == 0:
...
def main():
    ...
    root_02 = create_cylinder('ch01_0000_0000', 10, 10)
    ...
    last_ch_03_cylinder_list = cmds.listRelatives('|ch03_000_0000_rig_model', allDescendents=True, f=True,
                                                  type='transform')

    cmds.parent('|ch01_0000_0000|MESH|ch01_cylinder_3', w=True)
    cmds.move(-10, 0, 0, '|ch01_cylinder_3', relative=True)

    cmds.rename('|ch01_cylinder_3', 'ch01_cylinder_1')
    cmds.parent('|ch01_cylinder_1', '|ch03_000_0000_rig_model|MESH')
    ...
    cmds.parent('|ch00_0000_0000|joint_6', rig_model_root_group)
    cmds.rename('|ch03_000_0000_rig_model|joint_6', 'joint_0')

    cmds.move(20, 0, 0, rig_model_root_group)
    ...
    concentration_skin_weight.main('|ch03_000_0000_rig_model|MESH',
                                   '|ch03_000_0000_rig_model|joint_0|joint_1|joint_2')
